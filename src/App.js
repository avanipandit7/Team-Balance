...
import { collection, addDoc, onSnapshot, query, updateDoc, doc, serverTimestamp, orderBy, limit } from 'firebase/firestore';
...
export default function App() {
  ...
  const completeTaskWithEvidence = async (id) => {
    ...
    try {
      if (file) {
        ...
        const fileRef = ref(storage, `evidence/${id}_${Date.now()}_${file.name}`);
      ...
      console.error("Error completing task with evidence:", err);
      alert("Failed to save evidence. Please try again. " + err.message);
    }
  };
  ...
  if (!user) {
    return (
      <HeroGeometric
        badge='TeamBalance'
        title1='Balance the Work'
        title2='Make Collaboration Visible'
        description={isLogin
          ? 'Sign in to manage tasks, evidence, deadlines, and effort balance in one polished workspace.'
          : 'Create an account to start tracking ownership, progress, and contribution fairness across your team.'}
      >
        <div style={{ width: '100%' }}>
          {error && (
            <div style={{
          ...
          <form onSubmit={isLogin ? handleLogin : handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div>
                ...
                <input
                  ...
                  onFocus={(e) => {
                    ...
                    e.target.style.boxShadow = '0 0 0 4px rgba(56, 189, 248, 0.14)';
                  }}
                  onBlur={(e) => {
            ...
            <button
              ...
              onMouseEnter={(e) => {
                ...
                e.target.style.boxShadow = '0 18px 34px rgba(56, 189, 248, 0.25)';
          ...
          <div style={{
            ...
            <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: '0.9rem', margin: '0 0 0.75rem 0' }}>
            ...
            <button
              ...
              onMouseEnter={(e) => {
                e.target.style.backgroundColor = 'rgba(147, 197, 253, 0.12)';
                e.target.style.borderColor = 'rgba(147, 197, 253, 0.7)';
              }}
              onMouseLeave={(e) => {
                e.target.style.backgroundColor = 'transparent';
                e.target.style.borderColor = 'rgba(147, 197, 253, 0.35)';
              }}
            >
          ...
          <p style={{
            ...
            Secure Firebase auth powering your team workspace.
          </p>
        </div>
      </HeroGeometric>
    );
